...
% Nodes
\node (user) [startstop] {User / Client};
\node (api) [process, below of=user] {API Gateway / Load Balancer};
\node (server) [process, below of=api] {Web / Application Server (AWS / Render)};
\node (processing) [process, below of=server] {Data Processing Layer (PySpark)};
\node (database) [database, below of=processing] {Oracle Database};
\node (response) [startstop, below of=database] {Response sent back to User};

% Arrows
\draw [arrow] (user) -- (api);
\draw [arrow] (api) -- (server);
\draw [arrow] (server) -- (processing);
\draw [arrow] (processing) -- (database);
\draw [arrow] (database) -- (response);
...
\node[box=green, right=of user] (client) {Client App \\ (sends request)};
\node[box=orange, right=of client] (api) {API \\ (handles request/response)};
\node[box=red, right=of api] (server) {Server \\ (runs recommendation model)};
...
% Response arrows
\draw[arrow=red] (server.south) -- (response.north);
\draw[arrow=purple] (response.north) -- (api.south);

% Return arrows to client and user
...
\begin{enumerate}
    ...
    \item \textbf{Client App:} The client app collects the input and formats it into an API request (e.g., JSON) that is sent to the API.
    ...
    \item \textbf{Server:} The server runs the recommendation logic:
        \begin{itemize}
...
\subsection*{Notes on the Flow}
\begin{itemize}
    \item \textbf{User $\rightarrow$ Client $\rightarrow$ API $\rightarrow$ Server $\rightarrow$ Database:} This is the request path.
    \item \textbf{Database $\rightarrow$ Server $\rightarrow$ API $\rightarrow$ Client $\rightarrow$ User:} This is the response path.
...
All book related columns are mostly complete. The `book_author` and `publisher`
columns each have only 2 missing values out of 271,360 entries, while all other columns are fully populated.  
